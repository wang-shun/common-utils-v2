...
public class ShopAuthAspect extends BaseAspect {
    ...
    public Object handle(ProceedingJoinPoint pjp) {
        ...
        boolean hasShopPerms = checkPermission(needAppId, allowedPermissions, method.getDeclaringClass().getName() + "." + method.getName());
        if (hasShopPerms) {
    ...
    private boolean checkPermission(long appId, PermEnum[] allowedPermissions, String methodName) {
        ...
        for (PermEnum permissionsEnum : allowedPermissions) {
            if ((permissionsEnum.getPermInx().getIndex() + 1) > shopPermissions.length) {
                LOGGER.warn("interface permissions out of shop permissions,bid :{} shopId :{},need permission:{},shop permissions:{},method:{}", bid, shopId, permissionsEnum, shopPermissions,
                        methodName);
                return false;
            }
            ...
            if ((shopPermissions[permissionsEnum.getPermInx().getIndex()] & needPermission) != needPermission) {
                LOGGER.warn("interface permissions out of shop permissions,bid :{} shopId :{},need permission:{},shop permissions:{},method:{}", bid, shopId, permissionsEnum, Long.toHexString
                        (needPermission), methodName);
                return false;
            }
